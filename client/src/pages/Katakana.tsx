...
import { getKatakanaCharacters } from '@/lib/data';
import { KatakanaCharacter } from '@/types/learning';
...
export default function Katakana() {
  ...
  const katakanaCharacters = getKatakanaCharacters();
  const basicCharacters = katakanaCharacters.filter(char => char.category === 'basic');

  const quizQuestions = basicCharacters.slice(0, 10).map(char => ({
    question: `What is the romaji for this katakana character?`,
    options: [
      char.romaji,
    ...
    return (
      <div className="p-6 max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">Katakana Flashcards</h2>
        ...
        <div className="mt-4 bg-gray-100 rounded-full h-2">
          <div
            className="bg-n4 h-2 rounded-full transition-all duration-300"
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Katakana</h2>
          <p className="text-gray-600">Learn katakana for foreign words</p>
        </div>
        <div className="flex space-x-3">
          <Button
            onClick={() => setCurrentView('flashcard')}
            className="bg-n4 hover:bg-yellow-500"
      ...
      <Tabs defaultValue="basic" className="space-y-6">
        ...
        <TabsContent value="basic" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Katakana Character Chart</CardTitle>
            </CardHeader>
            <CardContent>
              <CharacterChart
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent>
                <img
                  ...
                  alt="Katakana practice charts"
                  className="w-full h-40 object-cover rounded-lg mb-4"
                />
                <p className="text-gray-600 text-sm mb-4">
                  Practice katakana characters used for foreign loanwords
                </p>
                <Button
                  onClick={() => setCurrentView('flashcard')}
                  className="w-full bg-n4 hover:bg-yellow-500"
            ...
            <Card>
              ...
              <CardContent>
                ...
                />
                <p className="text-gray-600 text-sm mb-4">
                  Match katakana characters with their pronunciations
                </p>
                <Button
                  onClick={() => setCurrentView('quiz')}
                  className="w-full"
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <p className="text-sm text-gray-500">
                  {learnedCharacters.size === 46 ? 'Congratulations! You\'ve mastered all katakana characters!' : `${46 - learnedCharacters.size} characters remaining`}
                </p>
              </div>
            </CardContent>
          </Card>
